...
Board::Board(string gameState)
{
	for (int i = 0; i < gameState.size(); i++)
	{
		...
		{
		case 'R':
			board[i / 8][i % 8 ]= new Rook(WHITE,this);
			break;
		case 'r':
			board[i / 8][i % 8] = new Rook(BLACK,this);
			break;
		case 'K':
			board[i / 8][i % 8] =new King(WHITE,this);
			break;
		case 'k':
			board[i / 8][i % 8] = new King(BLACK, this);
			break;
		case 'B':
			board[i / 8][i % 8] = new Bishop(WHITE, this);
			break;
		case 'b':
			board[i / 8][i % 8] = new Bishop(BLACK, this);
			break;
		case 'Q':
			board[i / 8][i % 8] = new Queen(WHITE, this);
			break;
		case 'q':
			board[i / 8][i % 8] = new Queen(BLACK, this);
			break;
		case 'P':
			board[i / 8][i % 8] = new Pawn(WHITE, this);
			break;
		case 'p':
			board[i / 8][i % 8] = new Pawn(BLACK, this);
			break;
		case 'N':
			board[i / 8][i % 8] = new Knight(WHITE, this);
			break;
		case 'n':
			board[i / 8][i % 8] = new Knight(BLACK, this);
			break;
		case '#':
 ...
 */
{
	...
	if (isSourceEmpty(src))             //   if there is not piece at the source
		...
		return 13;
	else if (!board[src.r][src.c]->isLegalMove(src, dest))
		return 21;
	...
	for (int i = 0; i < 8; i++)
		for (int j = 0; j < 8; j++)
			if ((board[i][j] != nullptr && (*board[i][j]).getColor() != color) // if there is here piece of the opposite
				&& (*board[i][j]).isLegalMove(Location(i, j), kingLocation))      // and it cause chess on the king
			{
				return true;
			}
	return false;
...
Piece* Board:: getValueFromMat(int r, int c)
/**
